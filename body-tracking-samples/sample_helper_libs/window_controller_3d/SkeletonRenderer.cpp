...
void SkeletonRenderer::SetJointRadius(float radius)
{
	if (radius > 0.0f)
	{
		m_jointRadius = radius;
		m_sphere.SetRadius(m_jointRadius);
	}
}

void SkeletonRenderer::SetBoneRadius(float radius)
{
	if (radius > 0.0f)
	{
		m_boneBaseRadius = radius;
		m_cylinder.SetBaseRadius(m_boneBaseRadius);
	}
}

void SkeletonRenderer::SetFrameAxisThickness(float thickness)
{
	if (thickness > 0.0f)
	{
		m_axisThickness = thickness;
		m_coordinateAxes.SetAxisThickness(m_axisThickness);
	}
}

void SkeletonRenderer::SetFrameAxisLength(float length)
{
	if (length > 0.0f)
	{
		m_axisLength = length;
		m_coordinateAxes.SetAxisLength(m_axisLength);
	}
}